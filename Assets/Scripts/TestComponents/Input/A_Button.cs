...
namespace Test.Input {
    ...
    public abstract class A_Button : MonoBehaviour {
        ...
        [SerializeField] private ButtonCondition _condition = None;
        /// <summary>
        /// 現在のボタンの状態(プロパティ)
        /// </summary>
        /// <value></value>
        ...
        /// </summary>
        /// <param name="token">キャンセルに要するトークン</param>
        /// <param name="waitTime"></param>
        /// <returns></returns>s
        private async UniTaskVoid PhaseHoldTimer (CancellationToken token, float waitTime) {
        ...
        /// <summary>
        /// 時間内にボタンが離されたならTrueをそれ以外ならfalseを返すタスク
        /// </summary>
        /// <param name="timeout"></param>
        /// <returns></returns>
        private async UniTask<Boolean> WaitButtonWasRelease(float timeout) {
            var tcs = new UniTaskCompletionSource();
            wasRelease.AddListener(OnTrigger);
            var completationTask = await UniTask.WhenAny(tcs.Task,UniTask.Delay(TimeSpan.FromSeconds(timeout)));
            
            if (completationTask == 1) return true;
            else return true;

            void OnTrigger(float dummy) {
                tcs.TrySetResult();
                wasRelease.RemoveListener(OnTrigger);
            }
        }
    }
}